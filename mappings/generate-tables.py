...
def get_presentation_destinations(
    varname: str, section: dict[str, Any], overrides: dict[str, Any]
):
    for dst in section:
        ...
        for section_idx, section in enumerate(mappings):
            ...
            content += dedent("""\
                        <table class="main">
                            <tr class="headerRow">
            """)
            for idx, (dst, clazz) in enumerate(
                get_presentation_destinations(varname, section, overrides)
            ):
                if idx > 0:
                    content += "\n"
            ...
            for idx, data in enumerate(
                get_presentation_mappings(varname, section, overrides)
            ):
                if idx > 0:
                    content += "\n"
...
def generate_implementation(impl: Impl):
    ...
    for idx, (lang, lang_info) in enumerate(impl.languages.items()):
        ...
        icon = f"IDI_ICON_{lang_info.icon.upper()}"
        compGuid = (
            f"&g_guidCompartment{lang.title()}"
            if len(lang_info.variants) > 1
            else "nullptr"
        )
...
def main():
    languages = ["be", "he", "ru", "uk", "uz"]
    for lang in languages:
        lang_config = MYDIR / f"{lang}.toml"
        with open(lang_config, "rb") as f:
            config = tomllib.load(f)
        icon = config.get("icon", lang)
        if not (ROOTDIR / "Translit" / "res" / f"{icon}.ico").exists():
            print(f"Icon {icon}.ico does not exist, please add", file=sys.stderr)
            return 1
    ...
    for language in languages:
        ...
        impl_lang = ImplLang(config["name"], config.get("icon", language), [])
